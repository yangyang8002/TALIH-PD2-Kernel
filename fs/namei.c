...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (inode && susfs_is_inode_sus_path((struct inode *)inode)) {
		return -ENOENT;
	}
#endif
			     ...
			     struct inode *dir, struct dentry *child)
{
	...
	if (child->d_inode && susfs_is_inode_sus_path(child->d_inode)) {
		int error = inode_permission(mnt_userns, dir, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
		return -ENOENT;
			...
			umode_t mode)
{
	...
	if (dentry->d_inode && susfs_is_inode_sus_path(dentry->d_inode)) {
		error = inode_permission(mnt_userns, dir->dentry->d_inode, MAY_WRITE | MAY_EXEC);
		if (error) {
			return error;
		}
		return -ENOENT;
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (!IS_ERR(filp) && SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(file_inode(filp))) {
		fake_pathname = susfs_open_redirect_spoof_do_sys_openat(file_inode(filp));
